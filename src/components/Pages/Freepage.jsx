...
import free from "../Games/freeData.json"

const Freepage = () => {
  return (
    <div className="w-[350px] md:w-[700px] lg:w-[950px] xl:w-[1180px] 2xl:w-[1440px] m-auto mt-10">
      <div className="flex flex-col items-center">
        <img src="img/cat1.png" alt="icon" className="w-[48px] h-[48px]" />
        <h3 className="text-[28px]">Free Games</h3>
      </div>
        ...
        {free.map((f) => (
          <div key={f.id}>
            <div className="flex flex-col gap-[10px]">
              <Game img={f.thumbnail} title={f.name} link={`/free/${f.id}`} />
            </div>
          </div>
        ))}
      </div>
